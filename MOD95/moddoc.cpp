...
BOOL CModuleDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
    ...
    if (pView)
        pView->PostMessage(WM_COMMAND, ID_VIDEO_PLAY);
    return TRUE;
}
...
{
    CMainFrame* pMainFrm = (CMainFrame*)AfxGetApp()->m_pMainWnd;
    if ((m_bPlay) || (!pMainFrm) || (!m_pSndFile))
        return;
    if (m_nVideoPos >= m_nVideoEnd - 1)
        m_nVideoPos = m_nVideoStart;
...
BOOL CModuleDoc::audioOpenDevice()
{
    ...
    if (!pView)
        return FALSE;
    if ((slSampleSize <= 0) || (slSampleSize > sdwAudioBufferSize))
        return FALSE;
    ...
    {
        if (!(salpAudioBuf[swBuffers] = (LPWAVEHDR)GlobalAllocPtr(GMEM_MOVEABLE | GMEM_SHARE, (DWORD)(sizeof(WAVEHDR) + sdwAudioBufferSize))))
            break;
...
BOOL CModuleDoc::audioiFillBuffers()
{
    ...
    if ((!shWaveOut) || (!m_pSndFile) || (!m_bPlay))
        return FALSE;
    while (swBuffersOut < swBuffers)
    {
        ...
        if (waveOutWrite(shWaveOut, salpAudioBuf[swNextBuffer], sizeof(WAVEHDR)) != MMSYSERR_NOERROR)
            return FALSE;
        ++swBuffersOut;
        ++swNextBuffer;
