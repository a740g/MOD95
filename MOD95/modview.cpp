...
LRESULT CModView::OnWOMDone(WPARAM, LPARAM)
{
...
LRESULT CModView::OnWOMIgnore(WPARAM, LPARAM)
{
    return 0;
}
...
void CModView::OnInitialUpdate()
{
    ...
    if (pDoc)
    {
        ...
        if (pMod)
        {
            ...
            UINT nmin = nsec / 60;
            nsec %= 60;
            sprintf_s(s, "\"%s\", %umn%us", text, nmin, nsec);
            for (int i = 1; i < 32; i++)
            {
...
LRESULT CModView::OnMoveSlider(WPARAM wParam, LPARAM)
{
    CModuleDoc* pDoc = GetDocument();
