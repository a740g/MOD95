...
constexpr auto MOD_PRECISION = 10;
constexpr auto MOD_FRACMASK = 1023;
constexpr auto MOD_AMIGAC2 = 0x1AB;
...
CSoundFile::CSoundFile(LPCSTR lpszPathName, UINT nRate, BOOL bHigh, BOOL bStereo, BOOL bLoop)
{
    ...
    if (!m_File.Open(lpszPathName, CFile::modeRead | CFile::typeBinary))
        return;
    m_nType = 1;
    ...
    if (!strcmp(s, "M.K."))
        m_nChannels = 4;
    else if ((s[0] == 'F') && (s[1] == 'L') && (s[2] == 'T') && (s[3] >= '1') && (s[3] <= '9'))
        m_nChannels = s[3] - '0';
    else if ((s[0] >= '1') && (s[0] <= '9') && (s[1] == 'C') && (s[2] == 'H') && (s[3] == 'N'))
        m_nChannels = s[0] - '0';
    else if ((s[0] == '1') && (s[1] >= '0') && (s[1] <= '6') && (s[2] == 'C') && (s[3] == 'H'))
        m_nChannels = s[1] - '0' + 10;
    else
        m_nSamples = 15;
    m_File.SeekToBegin();
    ...
    for (i = 1; i <= (int)m_nSamples; i++)
    {
        ...
        if (Ins[i].nVolume > 0x40)
            Ins[i].nVolume = 0x40;
        Ins[i].nVolume <<= 2;
        if ((j = ((unsigned int)bTab[26] << 9) | ((unsigned int)bTab[27] << 1)) < 4)
            j = 0;
        if ((k = ((unsigned int)bTab[28] << 9) | ((unsigned int)bTab[29] << 1)) < 4)
            k = 0;
        ...
        if (Ins[i].nLength)
        {
            if (j >= (int)Ins[i].nLength)
                j = Ins[i].nLength - 1;
            if (k > (int)Ins[i].nLength)
                k = Ins[i].nLength;
            if ((j > k) || (k < 4) || (k - j <= 4))
                j = k = 0;
        }
    ...
    for (i = 0; i < nbp; i++)
    {
        if ((Patterns[i] = (LPBYTE)GlobalAllocPtr(GHND, m_nChannels * 256)) == NULL)
            goto OnError;
        if (!m_File.Read(Patterns[i], m_nChannels * 256))
            goto OnError;
    }
    // Reading instruments
    for (i = 1; i <= (int)m_nSamples; i++)
...
UINT CSoundFile::Read(LPVOID lpBuffer, UINT cbBuffer)
{
    ...
    if ((!lMax) || (!p))
        return 0;
    if (m_nType == 1)
        return m_File.Read(lpBuffer, lMax * lSampleSize) / lSampleSize;
...
BOOL CSoundFile::ReadNote()
{
    if (!m_nSpeedCount)
    {
        ...
        for (UINT nChn = 0; nChn < m_nChannels; nChn++, p += 4)
        {
            ...
            if (instr > 31)
                instr = 0;
            if (instr)
                Chn[nChn].nNextIns = (short int)instr;
            if (period)
            {
            ...
            case 0x03:
                if (!param)
                    param = Chn[nChn].nPortamentoSlide;
            ...
            case 0x04:
                if (!bVib)
                    Chn[nChn].nVibratoPos = 0;
                if (param)
                    Chn[nChn].nVibratoSlide = param;
            ...
            case 0x05:
                ...
                {
                    Chn[nChn].nPortamentoDest = period;
                    if (Chn[nChn].nOldPeriod)
                        Chn[nChn].nPeriod = Chn[nChn].nOldPeriod;
                }
                Chn[nChn].bPortamento = TRUE;
                if (param)
                {
                    if (param & 0xF0)
                        Chn[nChn].nVolumeSlide = (int)((param >> 4) << 2);
                    else
                        Chn[nChn].nVolumeSlide = -(int)((param & 0x0F) << 2);
            ...
            case 0x07:
                if (!bTrem)
                    Chn[nChn].nTremoloPos = 0;
                if (param)
                    Chn[nChn].nTremoloSlide = param;
    ...
    for (UINT nChn = 0; nChn < m_nChannels; nChn++)
    {
        ...
        if (Chn[nChn].nVolume < 0)
            Chn[nChn].nVolume = 0;
        if (Chn[nChn].nVolume > 0x100)
            Chn[nChn].nVolume = 0x100;
        if (Chn[nChn].nCount1)
        {
            Chn[nChn].nCount1--;
            if (!Chn[nChn].nCount1)
                Chn[nChn].nPeriod = Chn[nChn].nPeriod1;
        }
        if (Chn[nChn].nCount2)
        {
            Chn[nChn].nCount2--;
            if (!Chn[nChn].nCount2)
                Chn[nChn].nPeriod = Chn[nChn].nPeriod2;
        }
        if (Chn[nChn].nPeriod)
        {
...
UINT CSoundFile::GetLength() const
{
